...
## 4.1 기능 정의 및 목적
* **목적**: 대표이사(CEO)가 시스템이 관리하는 핵심 도메인별 대표 현황 지표를 단 한 페이지에서 일목요연하게 조망하여 전체 사업 현황을 1초 만에 파악할 수 있는 의사결정용 요약 대시보드입니다.

---

## 4.2 데이터 모델 요건

대시보드 API는 핵심 도메인의 테이블들로부터 실시간 또는 스냅샷으로 통계 지표를 집계하여 전달합니다. 프로젝트는 M&A와 신사업을 별도 카드로 분리합니다.
...
interface CeoDashboardSummary {
  ...
  metrics: {
    ...
    // 9. 소속/조직 (Departments)
    totalDepartments: number;         // 활성 그룹/소속 수
...
대시보드는 **10개 현황 카드 섹션**과 **핵심 스케줄 목록** 두 영역으로 직관적이고 심플하게 구성합니다.

### 4.3.1 핵심 도메인 요약 그리드
반응형 카드 그리드(`1열 → 2열 → 3열`)로 주요 지표를 보여줍니다.
...
6.  **[6] M&A 카드**: 진행 중인 M&A 프로젝트 수
7.  **[7] 신사업 카드**: 진행 중인 신사업 프로젝트 수
8.  **[8] 후속 보고 카드**: `정기 보고서 제출율` (스타트업 컴플라이언스 이행 여부)
9.  **[9] 소속 본부 카드**: 활성 소속/그룹 수
10. **[10] 협력사 카드**: `등록 협력사 수` (대외 파트너십 네트워크 볼륨)
...
*   `system_events`를 기준으로 사업/프로젝트 마일스톤, IR 피칭 데이, 대외 협력 미팅 등 다가오는 주요 일정을 타임라인 형식으로 요약 제공합니다.
*   사업 일정은 `business_events`, 프로젝트 일정은 `project_events` 등록·수정 시 `system_events`에 동기화합니다. 동일 일정을 양쪽에서 수동으로 중복 입력하지 않습니다.
