...
## Phase 2 — 대시보드 (읽기 전용)
* **목표**: 핵심 도메인 요약 카드(현재 10개 카드: 프로젝트는 M&A/신사업 분리) + 다가오는 일정이 실데이터로 표시.
* **선행**: Phase 1.
...
* **순서·핵심 태스크** (발주자 요청으로 **프로젝트를 사업보다 앞당기고, 사업을 마지막**으로 조정, 2026-06-17):
  ...
  7. ✅ **투자 자료실**([22_invest_archives.md](22_invest_archives.md)): 게시판형 CRUD(고정 공지·조회수 RPC·제목/본문 검색·조회수 정렬, 카테고리 UI 폐지) + 공통 첨부 카드. 수정/삭제=작성자 본인·Admin. (`0058`)
* **✅ 선결정 게이트 — 해결됨(2026-06-17~18)**: 담당 심사역을 **다대다로 확정**(작성자/담당자/관리자 3계층). 스타트업·프로젝트 모두 `*_managers` 조인 적용 완료(`view_department_stats` 등 집계 반영). 작성자 자동 담당자 편입은 `0054`로 폐지되어 현행은 작성자와 담당자를 분리 관리한다. 게시글 **삭제 권한 = 작성자+관리자**(프로젝트·사업 적용), **수정 권한 축소**는 보류 유지. 상세 [6_startups.md](6_startups.md) 6.5.
