...
>
> **담당자 다대다(`fund_managers`, 0047)**: 발주자 확정으로 펀드도 담당자(n명)를 배정한다(명칭은 전 도메인 "담당자" 통일, [PATTERNS.md](PATTERNS.md) 17장). 공통 `EntityManagersPanel`(`kind="fund"`) 재사용. 조회=전 직원, **배정 추가/해제=Admin 전용**(펀드는 Admin 도메인). 책임자(`created_by`)는 트리거로 담당자에 자동 편입되며 **해제 불가**(0048).
...
*   **담당자 연계(다대다)**: `fund_managers` 조인(`fund_id`, `manager_id`, `0047`) -> 펀드 담당 심사역 여러 명. 책임자(`created_by`)와 별개이며 책임자는 자동 편입(0048). 배정 추가/해제=Admin.
...
*   **담당자(다대다) 배정 패널 (Detail Area 3)**: `fund_managers` 기반 담당 심사역 패널(`EntityManagersPanel kind="fund"`). 책임자는 자동 편입(해제 불가), 추가/해제는 **Admin 전용**. ([PATTERNS.md](PATTERNS.md) 17장)
