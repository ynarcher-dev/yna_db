...
* **적용 마이그레이션**: `0031`(uploaded_files 확장·attachment purpose·삭제정책), 소속은 `0032`(sections 컬럼 신설) 동반. 이후 도메인은 `entity_type` 값만 추가(VARCHAR·CHECK 없음 → 마이그레이션 불필요): `project`·`fund`·`business`·`matching_program`·`invest_archive`.
...
* **적용**: 스타트업(참여 사업·프로젝트·투자받은 펀드) · 심사역(담당 스타트업·프로젝트·운영 사업) · 협력사(참여 프로젝트) · 소속(부서원·`view_department_stats`).
...
* **조인 전용 속성**: 관계에만 있는 값(예: 사업 참여의 **보육 상태**)과 **연동 해제** 액션은 추가 컬럼으로 붙이되, `dataSource`는 도메인 객체 배열로 두고 **상대 id → 조인행 Map** 으로 역참조해 렌더한다(`rowKey`=상대 id). 행 클릭=상세 이동, 인라인 컨트롤은 `e.stopPropagation()`으로 분리.
...
* **현재 적용 범위**: 표 형태(목록 동일 컬럼 + 연동 해제)는 스타트업·심사역·협력사·사업·프로젝트 상세의 매핑 패널에 적용(소속은 RelatedListCard 목록 유지, 표 전환은 후속). **테두리+(연동)+섹션 토글은 전 도메인 모든 매핑 카드에 적용**(Tag형 EntityManagersPanel·ProjectLinksPanel 포함). 도메인별 추가된 토글 키: 사업 `partners`, 스타트업 `managers/businesses/projects`, 심사역 `startups/projects/businesses`, 협력사 `projects/businesses`, 소속 `members/stats`(프로젝트는 `managers/startups/partners` 기존).
* **양방향 모두 편집**: 새 다대다는 양쪽 상세에서 매핑할 수 있다(예: `business_partners` — 사업 상세 `BusinessPartnersPanel` + 협력사 상세 `PartnerBusinessesPanel`). 정/역(데이터 소유 관점)은 관례상 "목록 주인" 쪽을 정방향으로 두되, **시각 표시(테두리+(연동))와 토글은 양쪽 동일**하다.
