...
*   **참여 사업 연계**: `business_startups` 조인 -> 참여 배치 사업(+보육 상태). **상세 역방향 패널 '참여 사업 (연동)'으로 노출** — 사업 목록과 동일한 표 형태(메타 5컬럼 제외) + 보육상태·연동해제 컬럼(양방향, PATTERNS 18·18.1).
...
*   **참여 사업 & 프로젝트 목록 (Detail Tab 3)**: 참여했던 지원 사업 및 진행 중인 M&A 딜/오픈이노베이션 이력 표출.
