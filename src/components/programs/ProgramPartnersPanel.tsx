...
import {
  useProgramPartners,
  useProgramPartnerMutations,
  type ProgramPartnerRow,
...
/**
 * 프로그램 참여 협력사(기관) 매핑 패널 (program_partners, 정방향=주인 테이블).
 * 협력사 목록과 동일한 표 형태 + 관리(연결 해제). 추가/해제는 전 직원. (정방향이라 테두리 강조 없음)
 */
export function ProgramPartnersPanel({ programId }: { programId: string }) {
  const toast = useAppToast();
  const navigate = useNavigate();
  const { rows, isLoading } = useProgramPartners(programId);
  const { data: partnerOptions = [] } = usePartnerOptions();
  const { add, remove } = useProgramPartnerMutations(programId);
  ...
  const metaByPartner = useMemo(() => {
    const m = new Map<string, ProgramPartnerRow>();
    rows.forEach((r) => {
  ...
  const handleRemove = (r: ProgramPartnerRow) => {
