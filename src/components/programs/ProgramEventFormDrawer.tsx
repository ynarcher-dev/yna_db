...
import { programEventSchema, type ProgramEventInput } from '@/schemas/programEvent';
import { useProgramEventMutations } from '@/hooks/useProgramEvents';
...
/**
 * 프로그램 일정 등록/수정 Drawer. 저장/삭제 시 system_events 동기화 트리거가 함께 동작한다.
 * 신규 등록 시 캘린더에서 클릭한 날짜(defaultDate)를 초기값으로 받을 수 있다.
 */
export function ProgramEventFormDrawer({
  open,
  programId,
  event,
  defaultDate,
  ...
  const { create, update, remove } = useProgramEventMutations(programId);
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<ProgramEventInput>({
    resolver: zodResolver(programEventSchema),
    mode: 'onBlur',
  ...
  const onSubmit = (values: ProgramEventInput) => {
    const opts = {
      onSuccess: () => {
