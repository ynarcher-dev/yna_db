...
import { PartnerProgramsPanel } from '@/components/partners/PartnerProgramsPanel';
import type { PartnerSections } from '@/lib/partnerSections';

/**
 * 협력사 상세 역방향 연계 블록 (양방향 매핑의 '쓰는 쪽').
 * 참여 프로젝트(project_partners)·참여 프로그램(program_partners)을 이곳에서 직접 매핑(연결·신규생성)한다.
...
export function PartnerRelatedBlocks({
  ...
  return (
    <>
      ...
      {sections.programs ? <PartnerProgramsPanel partnerId={partnerId} /> : null}
    </>
  );
}
