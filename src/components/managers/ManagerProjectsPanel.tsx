...
/**
 * 심사역 상세 "담당 프로젝트" 역방향 편집 패널 (project_managers '쓰는 쪽', 역할 없음).
 * 표시는 프로젝트 목록과 동일한 컬럼 + 연동 해제. (a) 기존 프로젝트 연결 또는 (b) 신규 생성·즉시 매핑.
 */
export function ManagerProjectsPanel({ managerId }: { managerId: string }) {
  ...
  const { add, remove } = useManagerProjectMutations(managerId);
  ...
  const metaByProject = useMemo(() => {
    const m = new Map<string, ManagerProjectRow>();
    rows.forEach((r) => {
  ...
  const handleAdd = (projectId: string, isNew = false) => {
    add.mutate(projectId, {
      onSuccess: () => {
        toast.success(isNew ? '새 프로젝트가 생성·연결되었습니다.' : '담당 프로젝트가 추가되었습니다.');
  ...
  const handleRemove = (r: ManagerProjectRow) => {
    toast.confirm('담당 해제', `'${r.project?.name || '프로젝트'}' 담당을 해제하시겠습니까?`, async () => {
      try {
  ...
  return (
    <div className="rounded-lg border border-yna-border bg-white p-6">
      ...
      {rows.length === 0 && !isLoading ? (
        <EmptyState message="담당 중인 프로젝트가 없습니다." />
      ) : (
        <Table<Project>
          rowKey="id"
          size="small"
