...
import {
  useBusinessStartups,
  useBusinessStartupMutations,
  type BusinessStartupRow,
...
import { BUSINESS_STARTUP_STATUS_OPTIONS } from '@/lib/labels';
import type { Startup } from '@/types/startup';
import type { BusinessStartupStatus } from '@/types/database';

/**
 * 사업 참여 스타트업 매핑 패널 (business_startups, 정방향=주인 테이블).
 * 스타트업 목록과 동일한 표 형태 + 보육 상태(인라인 변경)·연동 해제. 추가/상태변경/해제는 전 직원.
 */
export function BusinessStartupsPanel({ businessId }: { businessId: string }) {
  const toast = useAppToast();
  const navigate = useNavigate();
  const { participants: rows, isLoading } = useBusinessStartups(businessId);
  const { data: startupOptions = [] } = useStartupOptions();
  const { add, updateStatus, remove } = useBusinessStartupMutations(businessId);
  const [selected, setSelected] = useState<string | undefined>();
  const [status, setStatus] = useState<BusinessStartupStatus>('applied');
  ...
  const metaByStartup = useMemo(() => {
    const m = new Map<string, BusinessStartupRow>();
    rows.forEach((r) => {
  ...
  const handleStatusChange = (id: string, next: BusinessStartupStatus) => {
    updateStatus.mutate(
      { id, status: next },
  ...
  const handleRemove = (r: BusinessStartupRow) => {
        ...
        return m ? (
          ...
          <div onClick={(e) => e.stopPropagation()}>
            <Select
              size="small"
              className="w-32"
              options={BUSINESS_STARTUP_STATUS_OPTIONS}
              value={m.status}
              onChange={(v: BusinessStartupStatus) => handleStatusChange(m.id, v)}
            />
          </div>
        ) : null;
      },
    ...
    <div className="rounded-lg border border-yna-border bg-white p-6">
      ...
      <div className="flex items-center gap-2">
        ...
        <Select
          ...
          options={BUSINESS_STARTUP_STATUS_OPTIONS}
          value={status}
          onChange={(v: BusinessStartupStatus) => setStatus(v)}
        />
        <Button
          type="primary"
          icon={<HiOutlinePlus />}
